		...
		for {
			...
			for pos < len(b) {
				if pos+2 < len(b) && b[pos] == 0 {
					_val3 = binary.BigEndian.Uint32(b[pos:])
					if _val3 == 0 {
						if pos+3 < len(b) {
...
func (info AVCSPSInfo) Id() uint {
	return info.Id_
}
func (info AVCSPSInfo) ProfileIdc() uint {
	return info.ProfileIdc_
}
func (info AVCSPSInfo) LevelIdc() uint {
	return info.LevelIdc_
}
func (info AVCSPSInfo) ConstraintSetFlag() uint {
	return info.ConstraintSetFlag_
}
func (info AVCSPSInfo) MbWidth() uint {
	return info.MbWidth_
}
func (info AVCSPSInfo) MbHeight() uint {
	return info.MbHeight_
}
func (info AVCSPSInfo) CropLeft() uint {
	return info.CropLeft_
}
func (info AVCSPSInfo) CropRight() uint {
	return info.CropRight_
}
func (info AVCSPSInfo) CropTop() uint {
	return info.CropTop_
}
func (info AVCSPSInfo) CropBottom() uint {
	return info.CropBottom_
}
func (info AVCSPSInfo) Width() uint {
	return info.Width_
}
func (info AVCSPSInfo) Height() uint {
	return info.Height_
}
func (info AVCSPSInfo) FPS() uint {
	return info.FPS_
}
...
func ParseSPS(data []byte) (s AVCSPSInfo, err error) {
	data = RemoveH264orH265EmulationBytes(data)
	r := &libbufio.GolombBitReader{R: bytes.NewReader(data)}

	if _, err = r.ReadBits(8); err != nil {
		return
	}

	if s.ProfileIdc_, err = r.ReadBits(8); err != nil {
		return
	}

	// constraint_set0_flag-constraint_set6_flag,reserved_zero_2bits
	if s.ConstraintSetFlag_, err = r.ReadBits(8); err != nil {
		return
	}
	s.ConstraintSetFlag_ = s.ConstraintSetFlag_ >> 2

	// level_idc
	if s.LevelIdc_, err = r.ReadBits(8); err != nil {
		return
	}

	// seq_parameter_set_id
	if s.Id_, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}

	if s.ProfileIdc_ == 100 || s.ProfileIdc_ == 110 ||
		s.ProfileIdc_ == 122 || s.ProfileIdc_ == 244 ||
		s.ProfileIdc_ == 44 || s.ProfileIdc_ == 83 ||
		s.ProfileIdc_ == 86 || s.ProfileIdc_ == 118 {
	...
	if s.MbWidth_, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	s.MbWidth_++

	if s.MbHeight_, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
	s.MbHeight_++
	...
	if frame_cropping_flag != 0 {
		if s.CropLeft_, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if s.CropRight_, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if s.CropTop_, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
		if s.CropBottom_, err = r.ReadExponentialGolombCode(); err != nil {
			return
		}
	}

	s.Width_ = (s.MbWidth_ * 16) - s.CropLeft_*2 - s.CropRight_*2
	s.Height_ = ((2 - frame_mbs_only_flag) * s.MbHeight_ * 16) - s.CropTop_*2 - s.CropBottom_*2
	...
	if vui_parameter_present_flag != 0 {
		...
		if timing_info_present_flag != 0 {
			...
			s.FPS_ = uint(math.Floor(float64(time_scale) / float64(num_units_in_tick) / 2.0))
...
func NewCodecDataFromAVCDecoderConfRecord(record []byte) (*AVCDecoderConfRecord, *AVCSPSInfo, error) {
	recordInfo := AVCDecoderConfRecord{}
	if _, err := recordInfo.Unmarshal(record); err != nil {
		return nil, nil, err
	}
	if len(recordInfo.SPS) == 0 {
		return nil, nil, fmt.Errorf("h264parser: no SPS found in AVCDecoderConfRecord")

	}
	if len(recordInfo.PPS) == 0 {
		return nil, nil, fmt.Errorf("h264parser: no PPS found in AVCDecoderConfRecord")
	}

	spsInfo, err := ParseSPS(recordInfo.SPS[0])
	if err != nil {
...
func (self AVCDecoderConfRecord) Profile() uint8 {
	return self.AVCProfileIndication
}

func (self AVCDecoderConfRecord) Compatibility() uint8 {
	return self.ProfileCompatibility
}

func (self AVCDecoderConfRecord) Level() uint8 {
	return self.AVCLevelIndication
}

func (self AVCDecoderConfRecord) LengthSize() uint8 {
	return self.LengthSizeMinusOne
}

func (self AVCDecoderConfRecord) SPSBytes() [][]byte {
	return self.SPS
}

func (self AVCDecoderConfRecord) PPSBytes() [][]byte {
	return self.PPS
}
...
func (self *AVCDecoderConfRecord) Unmarshal(b []byte) (n int, err error) {
	...
	self.LengthSizeMinusOne = b[4]&0x03 + 1
