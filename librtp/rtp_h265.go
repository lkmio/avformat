...
func (m *H265Muxer) Input(data []byte, timestamp uint32, alloc func() []byte, write func([]byte)) int {
	type_ := data[0] >> 1 & 0x3F
	length := len(data)
	...
	if length < m.maxPayloadSize {
		...
		return splitPayloadData(data[2:], m.maxPayloadSize, func(payload []byte, start, end bool) {
			if start {
				m.fuHeader[2] = 0x80
			} else if !end {
				m.fuHeader[2] = 0x00
