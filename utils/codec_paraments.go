...
type AVCCodecData struct {
	codecData
	Record *libavc.AVCDecoderConfigurationRecord
}
...
type HEVCCodecData struct {
	codecData
	Record *libhevc.HEVCDecoderConfigurationRecord
}
...
func ParseAVCDecoderConfigurationRecord(data []byte) (CodecData, error) {
	configurationRecord := libavc.AVCDecoderConfigurationRecord{}
	if err := configurationRecord.Unmarshal(data); err != nil {
		return nil, err
	}

	sps, err := libavc.ParseSPS(configurationRecord.SPSList[0])
	if err != nil {
		return nil, err
	}
...
func ParseHEVCDecoderConfigurationRecord(data []byte) (CodecData, error) {
	configurationRecord := libhevc.HEVCDecoderConfigurationRecord{}
	if err := configurationRecord.Unmarshal(data); err != nil {
		return nil, err
	}

	sps, err := libhevc.ParseSPS(configurationRecord.SPSList[0])
	if err != nil {
		return nil, err
	}
...
func NewAVCCodecData(sps, pps []byte) (CodecData, error) {
	spsInfo, err := libavc.ParseSPS(sps)
	if err != nil {
		return nil, fmt.Errorf("h264parser: parse SPS failed(%s)", err)
	}

	recordInfo := libavc.AVCDecoderConfigurationRecord{
		SPSList: make([][]byte, 1),
		PPSList: make([][]byte, 1),
	}
...
func NewHEVCCodecData(vps, sps, pps []byte) (CodecData, error) {
	spsInfo, err := libhevc.ParseSPS(sps)
	if err != nil {
		return nil, fmt.Errorf("h265parser: parse SPS failed(%s)", err)
	}

	recordInfo := libhevc.HEVCDecoderConfigurationRecord{
		VPSList: make([][]byte, 1),
		SPSList: make([][]byte, 1),
		PPSList: make([][]byte, 1),
	}
